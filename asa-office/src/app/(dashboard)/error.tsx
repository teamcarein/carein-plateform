...
const STATUS_LABELS: Record<number, { title: string; hint: string }> = {
  ...
  404: { title: 'Introuvable',            hint: 'L\'élément demandé n\'existe pas ou a été supprimé.' },
  409: { title: 'Conflit',               hint: 'L\'opération entre en conflit avec l\'état actuel.' },
  422: { title: 'Données invalides',      hint: 'Vérifiez les informations saisies.' },
...
export default function DashboardError({
  ...
  useEffect(() => {
    console.error('[CareIN Console]', error)
  }, [error])
    ...
    <div className="flex-1 flex items-center justify-center p-8 min-h-[60vh]">
      <div className="max-w-md w-full text-center space-y-6">
        ...
        <div className="relative inline-flex">
          <div className="w-20 h-20 rounded-[20px] bg-red-50 flex items-center justify-center">
            <AlertTriangle size={36} className="text-red-400" />
          </div>
          <span className="absolute -top-2 -right-3 text-[11px] font-bold bg-red-500 text-white px-2 py-0.5 rounded-full">
            {status}
          </span>
        </div>
        ...
        <div className="flex items-center justify-center gap-3">
          <button
            onClick={() => router.back()}
            className="flex items-center gap-2 px-4 py-2.5 rounded-[10px] text-sm font-semibold border border-border hover:bg-[#F3FAF7] transition-colors"
          >
            <ArrowLeft size={14} />
            Retour
          </button>
          <button
            onClick={reset}
            className="flex items-center gap-2 px-4 py-2.5 rounded-[10px] text-sm font-semibold bg-[#00C896] text-white hover:bg-[#009B73] transition-colors"
          >
            <RefreshCw size={14} />
            Réessayer
          </button>
        ...
        {error.digest && (
          <p className="text-[10px] text-foreground/25 font-mono">ref: {error.digest}</p>
        )}
      </div>
    </div>
  )
